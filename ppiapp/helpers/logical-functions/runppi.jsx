...
const ppi = (I0, alphas, alphas_prime, betas, A = null, R = null, bs = null, qm = null, rl = null,
  ...
  Object.values(sorted_programs).map(s => {
    ...
    Object.values(relevant_indis).map(i => {
      P0_budget[i] += Bs[l] * p0[i] / temp_sum_p0
    })
    l += 1
  ...
  for (var t in Array.from({ length: T })) {
    ...
    let deltaIIns = Object.keys(R).filter(v => R[v] == 1).map(r => I[r] - It[r]).slice() // instrumental indicators' changes
    ...
    F = F.map((item) => item == 0 || isNaN(item) ? 10e-12 : item);
    // TODO: fix next line (delete and fix error)
    X = X.map((item) => item == 0 || isNaN(item) ? 10e-12 : item);
    // ### DETERMINE CONTRIBUTIONconsole.logS ###
    const changeF = [...Array(F.length).keys()].map(i => F[i] - Ft[i]) // change in benefits
    const changeX = [...Array(X.length).keys()].map(i => X[i].toFixed(11) - Xt[i].toFixed(11)) // change in actions

    const sign = [...Array(changeF.length).keys()].map(i => Math.sign(changeF[i] * changeX[i])) // direction of the next action
    ...
    // TODO: fix next line (delete and fix error)
    C = C.map((item) => item == 0 || isNaN(item) ? 10e-12 : item);
    // eslint-disable-next-line no-undef
    ...
    let gammas = Object.keys(cnorm).map(i => (betas[i] * (cnorm[i] + temp_sum_C / (temp_sum_P + 1))) / temP_exp_S.get(i)) // compute probability of succesful growth
    // TODO: fix next line (delete and fix error)
    gammas = gammas.map((item) => item === 0 || isNaN(item) ? 10e-12 : item);
    // eslint-disable-next-line no-undef
    ...
    const newI = I.slice() // compute potential new values
    ...
    // TODO: fix next line (delete and fix error)
    P0_budget = P0_budget.map((item) => item == 0 || isNaN(item) ? 10e-12 : item);
    // eslint-disable-next-line no-undef
